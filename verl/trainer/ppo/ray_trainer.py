...
def online_reweighting(data: DataProto, lb: float = 0.1, ub: float = 1.0, micro_batch_limit: int = 64) -> Tuple[DataProto, dict]:
    total_samples = len(data)
    idx_array = data.non_tensor_batch["index"]
    total_unique_samples = len(np.unique(idx_array))
    rewards = data.batch["token_level_rewards"].sum(dim=-1).detach().cpu().numpy()
    keep_ids = {uid for uid in np.unique(idx_array) if rewards[idx_array == uid].max() > lb and rewards[idx_array == uid].min() < ub}
    mask = np.isin(idx_array, list(keep_ids))
    kept_idxs = np.where(mask)[0]
    kept_count = len(kept_idxs)
    unique_kept_count = len(keep_ids)

    discarded_num = total_unique_samples - unique_kept_count

    if kept_count == 0:
        all_idxs = np.arange(total_samples)
        replace = total_samples < micro_batch_limit
        kept_idxs = np.random.choice(all_idxs, size=micro_batch_limit, replace=replace)
        pad_size = micro_batch_limit
        
    else:
        pad_size = (micro_batch_limit - kept_count % micro_batch_limit) % micro_batch_limit
        if pad_size > 0:
            pad_idxs = np.random.choice(kept_idxs, size=pad_size, replace=True)
            kept_idxs = np.concatenate([kept_idxs, pad_idxs])

    filter_info = {
        "discarded_sample": discarded_num,
        "padding_count": pad_size,
        "batch_size": len(kept_idxs)
    }
    return data.select_idxs(kept_idxs), filter_info
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        batch, filter_info = online_reweighting(data=batch, lb=self.config.online_reweighting.lb, ub=self.config.online_reweighting.ub, micro_batch_limit=self.config.actor_rollout_ref.actor.ppo_micro_batch_size_per_gpu * self.config.trainer.n_gpus_per_node * self.config.actor_rollout_ref.actor.ppo_mini_batch_size)
                        metrics.update({
                            "online_reweighting/discarded_sample": filter_info["discarded_sample"], 
                            "online_reweighting/padding_count": filter_info["padding_count"],
                            "online_reweighting/batch_size": filter_info["batch_size"],
                            })
